...
export function registerSelectionCommands(plugin: VoloPlugin) {
  // ribbon 图标（桌面端可见；iOS 在 ribbon 菜单中仍可触达）
  plugin.addRibbonIcon("sparkles", "Volo：选中文本操作", async () => {
  ...
  for (const preset of SELECTION_PRESETS) {
    plugin.addCommand({
      id: `volo-selection-${preset.id}`,
      name: `Volo: 选中文本 → ${preset.label}`,
  ...
  // 自定义 prompt
  plugin.addCommand({
    id: "volo-selection-custom",
    name: "Volo: 选中文本 → 自定义 Prompt",
...
async function runSelectionAction(plugin: VoloPlugin, editor: Editor, selection: string, preset: { id: string; label: string; prompt: string }) {
...
async function runSelectionPrompt(
  ...
  const placeholder = String.fromCharCode(0x200b);
